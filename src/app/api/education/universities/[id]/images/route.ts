...
import {
  createEducationUniversityImageUseCase,
  deleteEducationUniversityImageUseCase,
  setEducationUniversityPrimaryImageUseCase,
...
export async function PATCH(
  ...
  try {
    ...
    await setEducationUniversityPrimaryImageUseCase(
      universityId,
      primary_image_id
    );
...
export async function DELETE(
  ...
  try {
    ...
    await deleteEducationUniversityImageUseCase(universityId, imageId);
    return NextResponse.json({ ok: true });
  } catch (error) {
    if (error instanceof EducationAccessError) {
      return NextResponse.json({ error: error.message }, { status: 403 });
