...
import { createEducationCourseImageUseCase } from "@/application/education";
...
export async function POST(
  ...
  try {
    ...
    for (const file of blobs) {
      ...
      const objectPath = `${courseId}/${uploadId}/${safeName}`;
      ...
      const row = await createEducationCourseImageUseCase(courseId, objectPath);
      rows.push(row);
    }
